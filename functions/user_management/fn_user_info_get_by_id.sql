-- Function: fn_user_info_get_by_id  |  Domain: user_management
...
RETURNS TABLE(
  ...
  reporting_rm VARCHAR, executive_name VARCHAR, district_of_posting VARCHAR, block_of_posting VARCHAR, state_of_posting VARCHAR,
  ...
  company_name VARCHAR, designation VARCHAR, project_id INT, project_name VARCHAR
) AS $$
BEGIN
  RETURN QUERY
  SELECT ui.user_info_id, ui.user_id, ui.company_id, ui.title,
    ...
    ui.reporting_rm, ui.executive_name, ui.district_of_posting, ui.block_of_posting, ui.state_of_posting,
    ...
    c.company_name, d.designation, ui.project_id, p.project_name
  ...
  LEFT JOIN project p ON p.project_id = ui.project_id
