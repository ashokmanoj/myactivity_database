...
RETURNS TABLE(synced_count INT, skipped_count INT)
LANGUAGE plpgsql
AS $$
DECLARE
  v_count   INT;
...
BEGIN
  ...
  -- Rows whose tripId doesn't exist are filtered out (via the EXISTS check) rather
  -- than left to hit the FK constraint — a single stale/invalid tripId in a batch
  -- must not abort the whole INSERT and silently drop every other valid point in it.
  WITH ranked AS (
    SELECT
      pt,
      NULLIF(pt->>'uuid', '')  AS p_uuid,
      (pt->>'tripId')::INT     AS p_trip_id,
      ROW_NUMBER() OVER (
  ...
  deduped AS (
    SELECT * FROM ranked WHERE p_uuid IS NULL OR rn = 1
  ),
  valid AS (
    SELECT d.* FROM deduped d
    WHERE EXISTS (SELECT 1 FROM distance_tracking dt WHERE dt.id = d.p_trip_id)
  ),
  inserted AS (
  ...
  SELECT
    ...
    (SELECT COUNT(*) FROM deduped) - (SELECT COUNT(*) FROM valid)
